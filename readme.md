...
- [Trust invariants](#trust-invariants) · [Severity](#severity)
- [Features](#features) · [Substantiation](#substantiation) · [Label & safety](#label--safety) · [Regulatory & competitive](#regulatory--competitive) · [Specialist claim lanes](#specialist-claim-lanes) · [Workspace](#workspace) · [DeepResearch lane](#deepresearch-lane)
...
| Citation currency & quality | Deterministic gate on age, citation count and source tier of every citation. No LLM, no extra calls | n/a |
...
| Export | Full review as JSON: findings, citations, decisions, audit |
...
| Surveillance-claim checker | "Cases are rising" and other trend claims, auto-routed here by the review and never faked inline | CDC wastewater / surveillance |
...
actually says what the claim says. That is entailment against the primary literature, with the
...
the key-based path rather than locking everyone out. The billing decision lives in one place, `src/lib/valyu-credentials.ts`,
...
3. **Point the app at the database.** On the *app* service → **Variables** → add
   ...
   `.railway.internal` host, since `db:init` detects it and skips TLS that the
   internal network doesn't need.
